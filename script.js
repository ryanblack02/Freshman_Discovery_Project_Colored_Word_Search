/* ===========================
   Neon Word Search - Complete script.js
   =========================== */
...
const words = {
  mammals: {
    kid: ["Dog","Cat","Cow","Horse","Pig","Sheep","Goat","Rabbit","Lion","Tiger",
          ...
          "Hedgehog","Bat","Raccoon","Cougar","Jaguar","Moose","Alpaca","Llama","Antelope","Beaver",
          "Hamster","Ferret","Chinchilla","Platypus","Wallaby","Reindeer","Bison","Hippopotamus","Porcupine"],
    adult: ["Aardvark","Aardwolf","Alpaca","Antelope","Bison","Capybara","Chimpanzee",
            ...
            "Wolverine","Gazelle","Caracal","Serval","Margay","Bushbaby","Tamarin",
            "Kinkajou","Ocelot","Baboon","Capuchin","Mandrill"]
  },
  birds: {
    kid: ["Chicken","Duck","Goose","Swan","Owl","Eagle","Hawk","Parrot","Penguin",
          "Flamingo","Peacock","Crow","Sparrow","Robin","Hummingbird","Toucan",
          "Woodpecker","Seagull","Pelican","Ostrich","Pigeon","Canary","Dove","Condor","Falcon"],
    adult: ["Albatross","Cassowary","Cockatoo","Condor","Crane","Falcon","Heron",
            "Hornbill","Kingfisher","Kookaburra","Macaw","Nightingale","Quail",
            "Roadrunner","Shoebill","Ibis","Bittern","Jay","Magpie","Wagtail"]
  },
  reptiles: {
    kid: ["Snake","Lizard","Turtle","Crocodile","Alligator","Chameleon","Gecko","Iguana","Skink","Anole","Tortoise"],
    adult: ["Komododragon","GarterSnake","Boa","Python","Anole","Tortoise","Monitor","Chameleon","Caiman","Agama","Viper","Gekko"]
  },
  amphibians: {
    kid: ["Frog","Toad","Salamander","Newt","Treefrog","Axolotl","Caecilian"],
    adult: ["Axolotl","Caecilian","Hellbender","Olm","Siren"]
  },
  fish: {
    kid: ["Goldfish","Shark","Tuna","Trout","Salmon","Clownfish","Catfish","Guppy","Betta","Mackerel"],
    adult: ["Barracuda","Grouper","Swordfish","Anglerfish","Lionfish","Betta","Piranha","Marlin","Sturgeon","Halibut","Carp"]
  },
  insects: {
    kid: ["Ant","Bee","Butterfly","Beetle","Fly","Wasp","Grasshopper","Ladybug","Moth","Cicada","Dragonfly"],
    adult: ["Dragonfly","Cricket","Termite","Moth","Mosquito","PrayingMantis","Firefly","Cockroach","Earwig","Lacewing"]
  },
  invertebrates: {
    kid: ["Snail","Worm","Jellyfish","Crab","Octopus","Starfish","Clam","Shrimp"],
    adult: ["Lobster","Scorpion","Coral","Anemone","Cuttlefish","Squid","Urchin","Barnacle","SeaSlug"]
  }
};
...
function showTemp(r1,c1,r2,c2){
  ...
  while(true){
    ...
    const el=cellElements[r][c]; if(el) el.classList.add("temp");
    if(r===r2&&c===c2) break;
    r+=dr;c+=dc;
  }
}
...
function enableInteraction(grid){
  ...
  const down = e => {
    isPointerDown=true;
    const p=getPointer(e); const cell=getCellFromPoint(p.clientX,p.clientY);
    if(!cell) return;
    pointerStart=cell;
    makeSparkle(p.clientX,p.clientY,"#fff");
    e.preventDefault();
  };
  const move = e => {
    if(!isPointerDown) return;
    const p=getPointer(e); makeSparkle(p.clientX,p.clientY,"#fff8");
    const cell=getCellFromPoint(p.clientX,p.clientY);
    if(!cell) return;
